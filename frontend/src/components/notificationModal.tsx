...
export default function NotificationModal() {
  ...
  const handleReject = (id: string) => {
    setNotifications((prev) =>
      prev.map((notif) => (notif.id === id ? { ...notif, status: "rejected" as const } : notif)),
    )
  }
    ...
    <Dialog>
      ...
      <DialogContent className="max-w-md p-0 gap-0 bg-gray-50 shadow-md">
        <DialogHeader className="p-6 pb-4 border-b">
          <DialogTitle className="text-xl font-semibold">Skill Swap Notifications</DialogTitle>
        </DialogHeader>
          ...
          {filteredNotifications.length === 0 ? (
            ...
            <div className="p-4 space-y-4">
              {filteredNotifications.map((notification) => (
                <div key={notification.id} className="bg-white border rounded-lg p-4 hover:shadow-sm transition-shadow">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <Badge
                          ...
                          className={notification.status === "pending" ? "bg-yellow-100 text-yellow-600" : ""}
                        >
                      ...
                      {notification.status === "pending" && (
                        <div className="flex gap-2">
                          ...
                          <Button size="sm"  className="border-red-800 text-red-800 hover:text-red-800 hover:bg-red-100" variant={"outline"} onClick={() => handleReject(notification.id)}>
                            Reject
                          </Button>
                        </div>
                      )}
